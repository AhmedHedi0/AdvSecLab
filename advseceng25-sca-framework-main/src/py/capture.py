...
def _setup_cwlite_cw305_100t() -> tuple[cw.scopes.OpenADC, cw.targets.CW305]:
    ...
    # ...
    #
    print(os.path.exists(r"C:\Users\Ahmed\Desktop\AdvSecLab\advseceng25-sca-framework-main\out\cw305.bit"))

    target = cw.targets.CW305()
    target._con(scope, bsfile=r"C:\Users\Ahmed\Desktop\AdvSecLab\advseceng25-sca-framework-main\out\cw305.bit",defines_files=[r"C:\Users\Ahmed\Desktop\AdvSecLab\advseceng25-sca-framework-main\src\hdl\cw305\params.vh"
...
class DutIOTestPattern(DutIOPattern):
    ...
    def next(self) -> DutIO:
        return DutIO(
            data=_cryptgen.randrange(0, 2**128),
            key= self.key,
            computed_data=None)
...
def capture_trace(scope:cw.scopes.OpenADC, target:cw.targets.CW305, ktp:DutIOPattern):
    ...
    for i_rep in range(ktp.average_over):
        ...
        dut_io.computed_data = int.from_bytes(bytes(dut_computed_data), byteorder='big')
        # Verify output
        expected_out = int.from_bytes(
    AES.new(bytes(key_in_bytes), AES.MODE_ECB).encrypt(bytes(data_in_bytes)),
    byteorder='big')

        print(f"Key (hex): {bytes(key_in_bytes).hex()}")
        print(f"Data (hex): {bytes(data_in_bytes).hex()}")
        print(f"DUT output raw: {dut_computed_data}")
        print(f"Expected output (hex): {expected_out.to_bytes(16, 'big').hex()}")
