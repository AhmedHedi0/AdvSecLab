...
This script demonstrates the complete side-channel analysis framework,
including trace capture, SNR analysis, attack implementation, and key extraction.
"""

import numpy as np
import time
from typing import Dict, List, Tuple
from sca_framework import TraceCapture, TraceVisualizer, HammingWeightModel, HammingDistanceModel
...
class FrameworkTester:
    ...
    def test_trace_capture_and_loading(self) -> List:
        ...
        try:
            ...
            if traces:
                ...
                print(f"✓ All traces same length: {len(set(trace_lengths)) == 1}")
    ...
    def test_snr_analysis(self, traces: List, correct_key: int) -> Dict:
        ...
        try:
            ...
            snr_empirical = self.snr_analyzer.calculate_snr_empirical(traces, correct_key, 0)
            ...
            # Find leakage points
            leakage_points = self.snr_analyzer.find_leakage_points(snr_empirical, threshold=0.1)
            print(f"✓ Found {len(leakage_points)} leakage points")
            
            # Estimate attack complexity
            complexity = self.snr_analyzer.estimate_attack_complexity(snr_empirical)
            print(f"✓ Attack feasibility: {complexity['attack_feasibility']}")
            print(f"✓ Estimated CPA traces needed: {complexity['estimated_cpa_traces']}")
            print(f"✓ Estimated DPA traces needed: {complexity['estimated_dpa_traces']}")
    ...
    def test_cpa_attack(self, traces: List, correct_key_byte: int) -> List:
        ...
        try:
            ...
            if cpa_results:
                ...
                print(f"✓ CPA attack completed with {len(cpa_results)} incremental results")
                print(f"✓ Best recovered key: 0x{best_result.recovered_key[0]:02x}")
    ...
    def test_dpa_attack(self, traces: List, correct_key_byte: int) -> List:
        ...
        try:
            ...
            if dpa_results:
                ...
                print(f"✓ DPA attack completed with {len(dpa_results)} incremental results")
                print(f"✓ Best recovered key: 0x{best_result.recovered_key[0]:02x}")
                print(f"✓ Correct key: 0x{correct_key_byte:02x}")
                print(f"✓ Attack success: {'YES' if success else 'NO'}")
    ...
    def test_visualizations(self) -> None:
        ...
        print("\n" + "="*60)
        print("PHASE 6: VISUALIZATION TEST")
        print("="*60)
        
        try:
            # Test SNR visualization
            if 'snr' in self.results:
                print("✓ Generating SNR analysis plots...")
                snr_data = self.results['snr']
                self.visualizer.plot_snr_analysis(snr_data['snr_values'], 
                                                 "Framework Test - SNR Analysis")
            ...
            if 'cpa' in self.results and 'dpa' in self.results:
                ...
                self.visualizer.plot_attack_comparison(self.results['cpa'], 
                                                     self.results['dpa'],
                                                     correct_key_bytes[0],
                                                     "Framework Test - Attack Comparison")
            
            # Test correlation heatmap
            if 'cpa' in self.results:
                print("✓ Generating correlation heatmap...")
                final_correlations = self.results['cpa'][-1].correlation_traces
                self.visualizer.plot_correlation_heatmap(final_correlations,
                                                        "Framework Test - Correlation Heatmap")
    ...
    def generate_final_report(self) -> str:
        ...
        report = []
        report.append("ChipWhisperer Side-Channel Analysis Framework Test Report")
        report.append("=" * 60)
        ...
        if 'snr' in self.results:
            ...
            report.append(f"  - Attack feasibility: {snr['complexity']['attack_feasibility']}")
            report.append("")
        ...
        if 'cpa' in self.results:
            ...
            report.append(f"  - Recovered key: 0x{best_cpa.recovered_key[0]:02x}")
        ...
        if 'dpa' in self.results:
            ...
            report.append(f"  - Recovered key: 0x{best_dpa.recovered_key[0]:02x}")
            report.append(f"  - Correct key: 0x{correct_key_byte:02x}")
        ...
        if 'full_key' in self.results:
            ...
            report.append(f"  - Bytes tested: {len(full_key['recovered_bytes'])}")
            report.append(f"  - Correct recoveries: {full_key['correct_recoveries']}")
        ...
        report.append(f"  - Framework Status: {'FUNCTIONAL' if overall_success else 'NEEDS IMPROVEMENT'}")
        report.append(f"  - Key Recovery: {'SUCCESSFUL' if overall_success else 'FAILED'}")
        report.append(f"  - Recommended Algorithm: {'CPA' if 'cpa' in self.results else 'DPA'}")
        report.append("")
...
def main():
    ...
    try:
        ...
        if traces:
            ...
            full_key_results = tester.test_full_key_recovery(traces, correct_key)
