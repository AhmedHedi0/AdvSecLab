"""
ChipWhisperer Side-Channel Analysis Framework
============================================

A comprehensive framework for performing side-channel attacks on AES implementations
using the ChipWhisperer platform.

"""

import os
import time
import numpy as np
import matplotlib.pyplot as plt
from typing import List, Tuple, Optional, Dict, Any
from dataclasses import dataclass, field
from abc import ABC, abstractmethod
import logging

# Configure logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s')
logger = logging.getLogger(__name__)

@dataclass
class TraceData:
    """Container for power trace data and associated metadata."""
    wave: np.ndarray
    plaintext: int
    key: int
    metadata: Dict[str, Any] = field(default_factory=dict)

@dataclass
class AttackResult:
    """Container for attack results."""
    recovered_key: np.ndarray
    correlation_traces: np.ndarray
    key_rankings: np.ndarray
    success_rate: float
    num_traces_used: int
    attack_type: str

class PowerModel(ABC):
    """Abstract base class for power consumption models."""
    
    @abstractmethod
    def compute(self, data: int, key_byte: int) -> float:
        """Compute the hypothetical power consumption."""
        pass

class HammingWeightModel(PowerModel):
    """Hamming weight power model for last round attack (using InvSBox)."""
    
    def __init__(self):
        self.sbox = self._generate_sbox()
    
    def _generate_sbox(self) -> np.ndarray:
        """Generate the AES inverse S-box."""
        sbox = np.zeros(256, dtype=np.uint8)
        # Standard AES inverse S-box values
        sbox_values = [
            
        ]
        for i, val in enumerate(sbox_values):
            sbox[i] = val
        return sbox
    
    def compute(self, ciphertext_byte: int, key_byte: int) -> float:
        """Compute Hamming weight of InvSBox output of ciphertext XOR key guess."""
        intermediate = self.sbox[ciphertext_byte ^ key_byte]
        return bin(intermediate).count('1')

class HammingDistanceModel(PowerModel):
    """Hamming distance power model."""
    
    def __init__(self, reference_value: int = 0):
        self.reference_value = reference_value
        self.sbox = HammingWeightModel()._generate_sbox()
    
    def compute(self, data: int, key_byte: int) -> float:
        """Compute Hamming distance from reference value."""
        sbox_output = self.sbox[data ^ key_byte]
        return bin(sbox_output ^ self.reference_value).count('1')

class TraceCapture:
    """Handles power trace capture from ChipWhisperer hardware."""
    
    def __init__(self, bitstream_path: Optional[str] = None):
        self.scope = None
        self.target = None
        self.bitstream_path = bitstream_path
        self.traces: List[TraceData] = []
        
    def setup_hardware(self) -> bool:
        """Initialize and configure ChipWhisperer hardware."""
        try:
            import chipwhisperer as cw
            
            # Setup scope
            self.scope = cw.scopes.OpenADC()
            self.scope.con(idProduct=0xace2, prog_speed=int(10E6))
            
            # Configure scope parameters
            self.scope.gain.db = 20
            self.scope.adc.basic_mode = "rising_edge"
            self.scope.adc.offset = 0
            self.scope.adc.presamples = 0
            self.scope.adc.samples = 150
            self.scope.clock.adc_src = "extclk_x4"
            
            # Configure IO
            self.scope.io.tio1 = "serial_rx"
            self.scope.io.tio2 = "serial_tx"
            self.scope.io.hs2 = "disabled"
            self.scope.trigger.triggers = "tio4"
            
            # Setup target
            self.target = cw.targets.CW305()
            self.target._con(self.scope, bsfile=self.bitstream_path, force=True, fpga_id='100t')
            self.target.vccint_set(1.0)
            
            # Configure target clock
            self.target.pll.pll_enable_set(True)
            self.target.pll.pll_outenable_set(False, 0)
            self.target.pll.pll_outenable_set(True, 1)
            self.target.pll.pll_outenable_set(False, 2)
            self.target.pll.pll_outfreq_set(7.37E6, 1)
            self.target.clkusbautooff = True
            self.target.clksleeptime = 1
            
            # Lock ADC
            self._lock_adc()
            
            logger.info("Hardware setup completed successfully")
            return True
            
        except Exception as e:
            logger.error(f"Hardware setup failed: {e}")
            return False
    
    def _lock_adc(self) -> None:
        """Lock ADC to external clock."""
        for _ in range(5):
            self.scope.clock.reset_adc()
            time.sleep(1)
            if self.scope.clock.adc_locked:
                logger.info("ADC locked successfully")
                return
        raise Exception("ADC failed to lock")
    
    def capture_single_trace(self, plaintext: int, key: int) -> Optional[TraceData]:
        """Capture a single power trace."""
        try:
            # Convert to bytes
            plaintext_bytes = list(plaintext.to_bytes(16, 'big'))
            key_bytes = list(key.to_bytes(16, 'big'))
            
            # Write inputs to target
            self.target.fpga_write(0x09, plaintext_bytes)  # Data input
            self.target.fpga_write(0x08, key_bytes)        # Key input
            
            # Trigger capture
            self.scope.arm()
            if self.target.clkusbautooff:
                self.target.usb_clk_setenabled(False)
            time.sleep(0.001)
            self.target.usb_trigger_toggle()
            if self.target.clkusbautooff:
                time.sleep(self.target.clksleeptime / 1000.0)
                self.target.usb_clk_setenabled(True)
            
            # Wait for capture
            if self.scope.capture():
                logger.warning("Scope capture timed out")
                return None
            
            # Wait for target completion
            retry_count = 0
            while self.target.fpga_read(0x05, 1)[0] == 0x01:
                retry_count += 1
                time.sleep(0.05)
                if retry_count > 100:
                    logger.warning("Target did not complete in time")
                    return None
            
            # Read result
            ciphertext_bytes = self.target.fpga_read(0x0A, 16)
            ciphertext = int.from_bytes(ciphertext_bytes, 'big')
            
            # Get trace
            wave = self.scope.get_last_trace()
            if len(wave) == 0:
                logger.warning("Empty trace received")
                return None
            
            # Create trace data object
            trace_data = TraceData(
                wave=wave,
                plaintext=plaintext,
                key=key,
                ciphertext=ciphertext,
                timestamp=time.time(),
                metadata={'trig_count': self.scope.adc.trig_count}
            )
            
            return trace_data
            
        except Exception as e:
            logger.error(f"Trace capture failed: {e}")
            return None
    
    def capture_traces(self, num_traces: int, key: int, 
                      progress_callback: Optional[callable] = None) -> List[TraceData]:
        """Capture multiple power traces with random plaintexts."""
        traces = []
        
        for i in range(num_traces):
            # Generate random plaintext
            plaintext = np.random.randint(0, 2**128)
            
            # Capture trace
            trace = self.capture_single_trace(plaintext, key)
            if trace is not None:
                traces.append(trace)
                self.traces.append(trace)
            
            # Progress callback
            if progress_callback and (i + 1) % 100 == 0:
                progress_callback(i + 1, num_traces)
        
        logger.info(f"Captured {len(traces)} traces out of {num_traces} attempts")
        return traces
    
    def save_traces(self, traces: List[TraceData], filename: str) -> None:
        """Save traces to compressed numpy file."""
        waves = np.array([trace.wave for trace in traces])
        plaintexts = np.array([trace.plaintext for trace in traces])
        keys = np.array([trace.key for trace in traces])
        ciphertexts = np.array([trace.ciphertext for trace in traces])
        timestamps = np.array([trace.timestamp for trace in traces])
        
        np.savez_compressed(
            filename,
            waves=waves,
            plaintexts=plaintexts,
            keys=keys,
            ciphertexts=ciphertexts,
            timestamps=timestamps
        )
        logger.info(f"Saved {len(traces)} traces to {filename}")
    
    def load_traces(self, filename: str) -> List[TraceData]:
        """Load traces from .npz file with keys: wave, dut_io_data, dut_io_computed_data."""
        data = np.load(filename, allow_pickle=True)
        traces = []
        waves = data['wave']
        plaintexts = data['dut_io_data']
        keys = data['dut_io_computed_data']
        
        for i in range(len(waves)):
            trace = TraceData(
                wave=waves[i],
                plaintext=int(plaintexts[i]),
                key=int(keys[i])
            )
            traces.append(trace)
        
        logger.info(f"Loaded {len(traces)} traces from {filename}")
        return traces
    
    def cleanup(self) -> None:
        """Clean up hardware connections."""
        try:
            if self.scope:
                self.scope.dis()
            if self.target:
                self.target.dis()
            logger.info("Hardware cleanup completed")
        except Exception as e:
            logger.error(f"Cleanup failed: {e}")

class TraceVisualizer:
    """Handles visualization of power traces and analysis results."""
    
    def __init__(self):
        plt.style.use('default')
        self.colors = ['#1f77b4', '#ff7f0e', '#2ca02c', '#d62728', '#9467bd', 
                      '#8c564b', '#e377c2', '#7f7f7f', '#bcbd22', '#17becf']
    
    def plot_traces(self, traces: List[TraceData], max_traces: int = 10, 
                   title: str = "Power Traces") -> None:
        """Plot multiple power traces."""
        plt.figure(figsize=(12, 8))
        
        num_traces = min(len(traces), max_traces)
        for i in range(num_traces):
            plt.plot(traces[i].wave, alpha=0.7, 
                    color=self.colors[i % len(self.colors)],
                    label=f'Trace {i+1}')
        
        plt.xlabel('Sample')
        plt.ylabel('Power Consumption')
        plt.title(title)
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
    
    def plot_average_trace(self, traces: List[TraceData], 
                          title: str = "Average Power Trace") -> None:
        """Plot average of multiple traces."""
        if not traces:
            logger.warning("No traces to plot")
            return
        
        waves = np.array([trace.wave for trace in traces])
        avg_wave = np.mean(waves, axis=0)
        std_wave = np.std(waves, axis=0)
        
        plt.figure(figsize=(12, 6))
        plt.plot(avg_wave, 'b-', linewidth=2, label='Average')
        plt.fill_between(range(len(avg_wave)), 
                        avg_wave - std_wave, 
                        avg_wave + std_wave, 
                        alpha=0.3, color='blue', label='±1 std')
        
        plt.xlabel('Sample')
        plt.ylabel('Power Consumption')
        plt.title(title)
        plt.legend()
        plt.grid(True, alpha=0.3)
        plt.tight_layout()
        plt.show()
    
    def plot_trace_statistics(self, traces: List[TraceData]) -> None:
        """Plot statistical analysis of traces."""
        if not traces:
            logger.warning("No traces to analyze")
            return
        
        waves = np.array([trace.wave for trace in traces])
        
        fig, axes = plt.subplots(2, 2, figsize=(15, 10))
        
        # Mean trace
        axes[0, 0].plot(np.mean(waves, axis=0), 'b-', linewidth=2)
        axes[0, 0].set_title('Mean Trace')
        axes[0, 0].set_xlabel('Sample')
        axes[0, 0].set_ylabel('Power')
        axes[0, 0].grid(True, alpha=0.3)
        
        # Standard deviation
        axes[0, 1].plot(np.std(waves, axis=0), 'r-', linewidth=2)
        axes[0, 1].set_title('Standard Deviation')
        axes[0, 1].set_xlabel('Sample')
        axes[0, 1].set_ylabel('Std Dev')
        axes[0, 1].grid(True, alpha=0.3)
        
        # Histogram of power values
        axes[1, 0].hist(waves.flatten(), bins=50, alpha=0.7, color='green')
        axes[1, 0].set_title('Power Value Distribution')
        axes[1, 0].set_xlabel('Power Value')
        axes[1, 0].set_ylabel('Frequency')
        axes[1, 0].grid(True, alpha=0.3)
        
        # Trace length distribution
        lengths = [len(trace.wave) for trace in traces]
        axes[1, 1].hist(lengths, bins=20, alpha=0.7, color='orange')
        axes[1, 1].set_title('Trace Length Distribution')
        axes[1, 1].set_xlabel('Trace Length')
        axes[1, 1].set_ylabel('Count')
        axes[1, 1].grid(True, alpha=0.3)
        
        plt.tight_layout()
        plt.show()

def progress_callback(current: int, total: int) -> None:
    """Default progress callback function."""
    percentage = (current / total) * 100
    print(f"Progress: {current}/{total} ({percentage:.1f}%)")

if __name__ == "__main__":
    # Example usage
    print("ChipWhisperer Side-Channel Analysis Framework")
    print("=" * 50)
    
    # Initialize trace capture
    capture = TraceCapture()
    
    # Note: Hardware setup would be done here in a real scenario
    # capture.setup_hardware()
    
    # Initialize visualizer
    visualizer = TraceVisualizer()
    
    print("Framework initialized successfully!")
    print("Ready for trace capture and analysis.")

